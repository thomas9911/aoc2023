...
#[pyfunction]
fn day01a(file_path: &str) -> PyResult<usize> {
...
#[pyfunction]
fn day01b(file_path: &str) -> PyResult<usize> {
    let file = File::open(file_path)?;
    let reader = BufReader::new(file);
...
fn aoc2023(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(day01a, m)?)?;
    m.add_function(wrap_pyfunction!(day01b, m)?)?;
    Ok(())
}
